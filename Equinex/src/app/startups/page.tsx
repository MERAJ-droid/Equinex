...
import { Search, Filter, Plus, Calendar, Target, Clock, ArrowRight, DollarSign, CheckCircle } from "lucide-react";
interface Campaign {
  owner: string;
  title: string;
  description: string;
...
export default function StartupsPage() {
  const { address, getCampaigns, fundStartup, withdrawStartupFunds } = useStarknetContext();
...
useEffect(() => {
  ...
  // Filter campaigns based on search term and exclude IP assets
useEffect(() => {
  let result = campaigns;
  
  // Filter out IP assets
  result = result.filter(campaign => !campaign.description.includes('#IPMETADATA'));
                  ...
                  <Card className="border-slate-700 bg-slate-800/50 backdrop-blur-sm overflow-hidden h-full flex flex-col">
                    <div className="relative aspect-video overflow-hidden">
                      ...
                      <div className="absolute top-3 left-3">
                        <VerificationBadge address={campaign.owner} showLabel={true} />
                      </div>
                    </div>
...
.filter((f: { funderAddress: string; amount: string; tokenId: string }) => 
  address && f.funderAddress.toLowerCase() === address.toLowerCase()
)
